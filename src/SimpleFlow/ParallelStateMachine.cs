...
namespace SimpleFlow.Core
{
    class ParallelStateMachine : IStateMachine //todo: combine with fork state machine
    {
        ...
        public void Transit(WorkItem workItem, IEngine engine)
        {
            ...
            if (workItem.Type != WorkflowType.Parallel) throw new ArgumentException("type must be pallaral");

            var status = workItem.Status;
            var definition = (ParallelBlock) _workflowPathNavigator.Find(workItem.WorkflowPath);
                ...
                case WorkItemStatus.WaitingForChildren:
                    ...
                    if (newWorkers > 0)
                    {
                        ...
                        else if (inProgress == 0)
                        {
                            ...
                            workItem.OutputId = _dataStore.AddReferences(workItem.JobId, ids, typeof (object));
